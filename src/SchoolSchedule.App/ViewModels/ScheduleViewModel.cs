...
public partial class ScheduleViewModel : ObservableObject
{
    ...
    [
        ...
        "«Уроков/день» в учебном плане ограничивает, сколько уроков предмета можно поставить в один день (1 по умолчанию — не больше одного в день).",
        "Если для предмета включены «Парные уроки» — когда в один день ставится несколько его уроков, они идут подряд, без окна между ними.",
        "Если предмет разбит на подгруппы у одного класса — общая часть часов встаёт в один и тот же урочный слот (учатся параллельно, с разными учителями и кабинетами); если у одной подгруппы часов в неделю больше, чем у другой, «лишние» уроки ставятся отдельно — в это время у другой подгруппы может не быть урока, и это нормально.",
        ...
        "Дальше — три уровня приоритета (не запрет, а «по возможности», от самого важного к наименее важному): 1) минимум окон (пустых уроков между занятиями в один день) у учеников — считается отдельно на весь класс и, если есть подгруппы, отдельно на каждую подгруппу; 2) минимум окон у учителей; 3) кабинет, закреплённый за учителем (страница «Кабинеты»), по возможности используется именно им, но при необходимости в нём может вести и другой учитель.",
        "На больших расписаниях (много классов и учителей) генерация может занимать несколько минут — это нормально, солвер ищет расписание с как можно меньшим числом окон, а не первое попавшееся.",
    ];
